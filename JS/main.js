...
function checkIfCanWin() {
    const arr = JSON.parse(JSON.stringify(board)); //trick to do a deep copy with arrays bidimensional if array is 1 dimension to do: [...]
    for (let row = 0; row < arr.length; row++) {
        for (let column = 0; column < arr.length; column++) {

            if (arr[row][column] == "X") {
                arr[row][column] = { value: 1, row, column };
            }

            if (arr[row][column] == "") {
                arr[row][column] = { value: 0, row, column };
            }

            if (arr[row][column] == "O") {
                arr[row][column] = { value: -2, row, column };
            }
